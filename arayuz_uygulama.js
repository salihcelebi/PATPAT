...
const state = {
  mode: 'analysis',
  statusFilters: ['pending','processing','completed','canceled','returnprocess','problematic'],
  searchQuery: '',
  pageLimit: 0,
  autoSend: false,
  dryRun: true,
...
function attachEvents() {
  // Eksik DOM elemanı varsa crash etme: logla ve mevcut olanlarla devam et.
  const requiredIds = [
    'modeSelect','statusSelect','searchInput','pageLimitInput','autoSendToggle','dryRunToggle',
    'btnStart','btnStop','btnExportCsv','btnExportJson',
    'templateSelect','btnGenerateTemplate','btnSendMessage','btnCopyMessage',
    'logLevelFilter','btnRetry','btnManual',
    'btnCopyAllLogs','btnExportLogs','btnToggleLogPanel','logToggle','logStateBtn',
    'btnQuickCopy','btnOpenMessagePage','btnOpenMessagePageQuick','btnReloadUi','btnFocusSelected'
  ];
  const missing = requiredIds.filter(id => !document.getElementById(id));
  if (missing.length) {
    LogManager.addLog({ level: 'error', module: 'ui', action: 'missing_dom', result: missing.join('|') });
    renderLogs();
  }
  ...
  if (refs.statusSelect) refs.statusSelect.addEventListener('change', () => {
    const selected = Array.from(refs.statusSelect.selectedOptions).map(o => o.value);
    state.statusFilters = selected;
  });

  if (refs.searchInput) refs.searchInput.addEventListener('input', () => {
    state.searchQuery = refs.searchInput.value.trim();
    renderOrdersTable();
  });

  if (refs.pageLimitInput) refs.pageLimitInput.addEventListener('input', () => {
    const v = parseInt(refs.pageLimitInput.value, 10);
    state.pageLimit = isNaN(v) ? 0 : Math.max(0, v);
  });

  if (refs.autoSendToggle) refs.autoSendToggle.addEventListener('change', () => {
    state.autoSend = refs.autoSendToggle.checked;
  });

  if (refs.dryRunToggle) refs.dryRunToggle.addEventListener('change', () => {
    state.dryRun = refs.dryRunToggle.checked;
    updateMessagePreview(refs.messagePreview?.value || '');
  });

  if (refs.btnStart) refs.btnStart.addEventListener('click', () => { startScan().catch(e => {
    LogManager.addLog({ level:'error', module:'ui', action:'startScan', result:'error', error:String(e?.message||e) });
    renderLogs();
  }); });

  if (refs.btnStop) refs.btnStop.addEventListener('click', () => { stopScan(); });

  if (refs.btnExportCsv) refs.btnExportCsv.addEventListener('click', () => { exportCsv(); });
  if (refs.btnExportJson) refs.btnExportJson.addEventListener('click', () => { exportJsonl(); });

  if (refs.templateSelect) refs.templateSelect.addEventListener('change', () => {
    if (selectedOrderId) {
  ...
  if (refs.btnGenerateTemplate) refs.btnGenerateTemplate.addEventListener('click', () => {
    if (selectedOrderId) {
      const orderRow = recordStore.orders.get(String(selectedOrderId));
      const smmRow = orderRow?.smm_id ? recordStore.smmOrders.get(String(orderRow.smm_id)) : null;
      const message = smmRow ? generateMessage(orderRow, smmRow, refs.templateSelect.value) : '';
  ...
  if (refs.btnSendMessage) refs.btnSendMessage.addEventListener('click', async () => {
  ...
  if (refs.btnCopyMessage) refs.btnCopyMessage.addEventListener('click', async () => {
    const txt = refs.messagePreview?.value || '';
    if (!txt) return;
    try {
      await navigator.clipboard.writeText(txt);
    } catch (e) {
      LogManager.addLog({ level:'warn', module:'ui', action:'copyMessage', result:'fallback', error:String(e?.message||e) });
      renderLogs();
    }
  });

  // LOGS: seviye filtresi
  if (refs.logLevelFilter) refs.logLevelFilter.addEventListener('change', () => { renderLogs(); });

  // LOGS: kopyala/export (kritik)
  const btnCopyAllLogs = document.getElementById('btnCopyAllLogs');
  if (btnCopyAllLogs) btnCopyAllLogs.addEventListener('click', async () => {
    const txt = LogManager.exportText({}); // düz metin
    try { await navigator.clipboard.writeText(txt || ''); }
    catch (e) { LogManager.addLog({ level:'error', module:'ui', action:'copyAllLogs', result:'error', error:String(e?.message||e) }); }
    renderLogs();
  ...
  if (btnExportLogs) btnExportLogs.addEventListener('click', () => {
    const txt = LogManager.exportJsonl({});
    const blob = new Blob([txt], { type: 'application/jsonl' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'patpat_logs.jsonl';
    a.click();
    URL.revokeObjectURL(url);
  });

  // Log panel toggle (gerçek kapanma)
  const btnToggleLogPanel = document.getElementById('btnToggleLogPanel');
  const logPanel = document.getElementById('logPanel');
  const logToggle = document.getElementById('logToggle');
  const logStateBtn = document.getElementById('logStateBtn');
  const toggleLog = () => {
    if (!logPanel) return;
    logPanel.classList.toggle('collapsed');
    if (logStateBtn) logStateBtn.textContent = logPanel.classList.contains('collapsed') ? 'AÇ ▼' : 'KAPAT ▲';
  };
  if (btnToggleLogPanel) btnToggleLogPanel.addEventListener('click', toggleLog);
  if (logToggle) logToggle.addEventListener('click', (e) => {
    if (e.target && String(e.target.id) === 'logStateBtn') return; // buton kendi handler'ıyla
    toggleLog();
  });
  if (logStateBtn) logStateBtn.addEventListener('click', toggleLog);

  // Quick actions
  const btnQuickCopy = document.getElementById('btnQuickCopy');
  if (btnQuickCopy) btnQuickCopy.addEventListener('click', async () => {
    const txt = refs.messagePreview?.value || '';
    if (!txt) return;
    try { await navigator.clipboard.writeText(txt); } catch {}
  });

  const btnOpenMessagePage = document.getElementById('btnOpenMessagePage');
  if (btnOpenMessagePage) btnOpenMessagePage.addEventListener('click', () => {
    const orderRow = selectedOrderId ? recordStore.orders.get(String(selectedOrderId)) : null;
    const buyer = String(orderRow?.buyer_username || '').trim();
    if (!buyer) return alert('Önce bir sipariş seç (müşteri kullanıcı adı yok).');
    const url = `https://hesap.com.tr/u/${encodeURIComponent(buyer)}`;
    window.open(url, '_blank');
  });

  const btnOpenMessagePageQuick = document.getElementById('btnOpenMessagePageQuick');
  if (btnOpenMessagePageQuick) btnOpenMessagePageQuick.addEventListener('click', () => {
    const orderRow = selectedOrderId ? recordStore.orders.get(String(selectedOrderId)) : null;
    const buyer = String(orderRow?.buyer_username || '').trim();
    if (!buyer) return alert('Önce bir sipariş seç (müşteri kullanıcı adı yok).');
    const url = `https://hesap.com.tr/u/${encodeURIComponent(buyer)}`;
  ...
  const btnReloadUi = document.getElementById('btnReloadUi');
  if (btnReloadUi) btnReloadUi.addEventListener('click', () => location.reload());

  const btnFocusSelected = document.getElementById('btnFocusSelected');
  if (btnFocusSelected) btnFocusSelected.addEventListener('click', () => {
    if (!selectedOrderId) return;
    const tr = refs.ordersBody?.querySelector?.(`tr[data-order-id="${selectedOrderId}"]`);
    if (tr) tr.scrollIntoView({ behavior:'smooth', block:'center' });
  });

  // Puter demo
  const btnPuterChatDemo = document.getElementById('btnPuterChatDemo');
  if (btnPuterChatDemo) btnPuterChatDemo.addEventListener('click', () => {
